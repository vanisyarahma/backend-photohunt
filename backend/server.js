...
app.get("/studios/:id/detail", async (req, res) => {
  try {
    ...
    // 2) Gallery (WAJIB ADA 'id' BIAR BISA DIHAPUS)
    const [images] = await db.query("SELECT id, image FROM studio_images WHERE studio_id = ?", [studioId]);
    ...
    const [reviewStats] = await db.query("SELECT COUNT(*) as totalReviews, AVG(rating) as avgRating FROM reviews WHERE studio_id = ?", [studioId]);
    ...
    res.json({
      studio: { ...studio, rating: avgRating, totalReviews: totalReviews },
      images,
      facilities,
      packages,
      schedules,
    ...
    if (packages) {
      for (const key in packages) {
        ...
        await db.execute(
          ...
          [studioId, p.name, p.price, p.description || null]
        );
      }
    }
...
app.put("/studios/:studioId", async (req, res) => {
  try {
    ...
    await db.execute(
      "UPDATE studios SET name=?, description=?, price_range=?, gmaps_link=? WHERE id=?",
      [name, description, price_range, gmaps_link, studioId]
    );
    ...
    res.json({ success: true, message: "Studio dan Jadwal berhasil diupdate" });
